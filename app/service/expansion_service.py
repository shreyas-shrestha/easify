...
class ExpansionService:
    def __init__(self, settings: Settings) -> None:
        ...
        self.cache_service: CacheService = open_cache_service(settings)
    ...
    async def _run_live_enrich_job(self, client: httpx.AsyncClient, job: LiveEnrichJob) -> None:
        ...
        if self.settings.live_enrich_skip_same and t.lower() == job.user_text.lower():
            return
        self.cache_service.store_live_enrich(model, job.cache_prompt, t)
        if self.metrics is not None:
        ...
        while True:
            ...
            try:
                ...
                cap = await run_capture_expand_async(
                    ...
                    citation_mode=job.citation_mode,
                )
                self._consume_consecutive_failures = 0
                ...
                if preview_act.type == ActionType.SHOW_PREVIEW:
                    from app.ui.preview import confirm_expansion

                    ok = await asyncio.to_thread(confirm_expansion, preview_act.text or "")
                    if not ok:
                        self._discard_pending_tail(job)
