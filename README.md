...
| Feature | Env / notes |
|---------|-------------|
| **Focused app → L3** | `EASIFY_CONTEXT_FOCUSED_APP=1` (default): macOS AppleScript, Windows title, Linux **X11** `xdotool` / `xprop`. **Wayland:** not supported — focus label stays `unknown` (see `easify doctor`). |
...
| **Keyboard backend** | `EASIFY_BACKEND=pynput` (default) \| `keyboard` (optional `pip install easify[keyboard]`) \| `evdev` + `EASIFY_EVDEV_DEVICE=/dev/input/eventN` (`pip install easify[evdev]`). **Linux / Wayland:** `xdotool` focus does not work; Easify logs this and focus stays `unknown` (namespaced snippets degrade unless `EASIFY_SNIPPET_NAMESPACE_LENIENT=1`). **evdev** needs read access to the event device (`input` group or udev); on failure Easify falls back to pynput and logs that native Wayland clients often block it. With the `keyboard` backend, **Esc** is forwarded to the listener (same as pynput) so capture cancel works. |
| **Accessibility inject** | `EASIFY_INJECT_ACCESSIBILITY=1` (default) + `pip install easify[accessibility]` — replace the capture span in the **focused** control’s string in one write (default **last** match; `EASIFY_INJECT_ACCESSIBILITY_MATCH_LAST=0` for **first`) (macOS `AXValue`, Windows `ValuePattern`). **`EASIFY_INJECT_ACCESSIBILITY_UNIQUE_MATCH_ONLY=1`** (default): require **exactly one** occurrence of the capture text in the **currently focused** field before AX/UIA replace — avoids wrong split-pane / multi-field edits when focus moved (**undo** still allows non-unique matches). **Password / secure fields:** detected on **macOS** (AX) and **Windows** (UIA) when deps are installed — live fixes and expansions **skip inject**. **Linux** has no portable secure-field API here yet. **Live autocorrect** skips tokens that look like **IME / CJK / Arabic / combining marks** (best-effort; Latin IME such as pinyin mid-composition is not visible to pynput). **No synthetic keys** on success. **Caret/selection** after `SetValue` is up to each app (not always preserved). Enable **Accessibility** for the process running Easify on macOS. If the substring is missing or the API fails, Easify **falls back** to keystroke inject **only after** verifying frontmost app matches submit-time context when that context is known (avoids backspacing into the wrong window). |
